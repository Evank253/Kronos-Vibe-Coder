...
def test_create_branch_existing(mock_gh_class):
    ...
    resp = github_manager.create_branch('owner/repo', 'new-branch', base_branch='main', token='tok')
...
def test_commit_changes_create_update(mock_gh_class):
    ...
    resp = github_manager.commit_changes('owner/repo', 'branch', 'msg', changes, token='tok')
...
def test_open_pr(mock_gh_class):
    ...
    resp = github_manager.open_pull_request('owner/repo', 'title', 'body', 'head', base='main', token='tok')
