...
def test_scan_repository_runs_pipeline(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda url: {"status": "cloned", "repository": "fake-repo", "path": str(fake_repo_dir)}
    )
    monkeypatch.setattr(
        "backend.agents.scan_pipeline.analyze_repo",
        lambda path: {"status": "analysis complete", "files_found": 1, "files": [str(fake_repo_dir / "README.md")]}
    )
    monkeypatch.setattr(
        "backend.agents.scan_pipeline.debug_project",
        lambda data: {"status": "debug scan complete", "issues_found": [], "message": "No errors detected yet"}
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda path: {"deployment_score": "0/5", "checks": {}, "ready": False}
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda report: {"review_status": "complete", "recommendations": [], "next_action": "Run debug and deployment agents"}
    )
    monkeypatch.setattr(
